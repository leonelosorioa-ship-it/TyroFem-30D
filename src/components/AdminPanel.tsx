...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onBackToApp, onLogoutAdmin }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans pb-16">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-6 space-y-6">
        ...
        <section className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-5 gap-3.5 sm:gap-4">
          ...
          <div 
            ...
            className="bg-slate-900/90 border border-cyan-900/60 hover:border-cyan-400/80 rounded-2xl p-4 shadow-md space-y-2 col-span-2 sm:col-span-2 lg:col-span-1 cursor-pointer transition-all hover:bg-slate-900/100 group"
            title="Click para ver y autorizar códigos VIP"
          >
            <div className="flex items-center justify-between text-cyan-300">
              <span className="text-xs font-semibold uppercase tracking-wider">Códigos VIP 6 Dígitos</span>
              <KeyRound className="w-4 h-4 text-cyan-400 group-hover:scale-110 transition-transform" />
            </div>
            <div className="flex items-baseline gap-2">
            ...
            <div className="flex items-center justify-between pt-0.5">
              ...
              <span className="text-[10px] text-cyan-400 underline font-medium">Ver y autorizar &gt;</span>
            </div>
          </div>
      ...
      {/* ========================================================================= */}
      {/* MODAL: GESTIÓN Y AUTORIZACIÓN DE CÓDIGOS VIP */}
      {/* ========================================================================= */}
      {isCodesModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/85 backdrop-blur-sm animate-fadeIn">
          <div className="bg-slate-900 border border-slate-700 rounded-3xl max-w-2xl w-full p-5 sm:p-6 space-y-4 shadow-2xl animate-scaleUp max-h-[90vh] flex flex-col">
            
            {/* Header */}
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div className="flex items-center gap-2.5">
                <div className="p-2 bg-cyan-950/80 border border-cyan-500/50 rounded-xl text-cyan-300">
                  <KeyRound className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="font-bold text-white text-base">
                    Códigos VIP Autorizados - Tyruss Full
                  </h3>
                  <p className="text-xs text-slate-400">
                    Base oficial de códigos autorizados para compradoras de ColShopi
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="grid grid-cols-3 gap-2 bg-slate-950/60 p-3 rounded-2xl border border-slate-800 text-center">
              <div>
                <span className="text-[10px] text-slate-400 font-semibold block uppercase">Total Autorizados</span>
                <span className="text-lg font-black text-white">{codesSummary.total}</span>
              </div>
              <div>
                <span className="text-[10px] text-cyan-400 font-semibold block uppercase">Canjeados / En uso</span>
                <span className="text-lg font-black text-cyan-300">{codesSummary.usedCount}</span>
              </div>
              <div>
                <span className="text-[10px] text-emerald-400 font-semibold block uppercase">Disponibles</span>
                <span className="text-lg font-black text-emerald-400">{codesSummary.availableCount}</span>
              </div>
            </div>
            ...
            <form 
              onSubmit={(e) => {
                e.preventDefault();
                const clean = newManualCodeToAuthorize.replace(/\D/g, '').trim();
                if (clean.length !== 6) {
                  setCodeAuthorizeMsg({ type: 'error', text: 'El código debe tener exactamente 6 dígitos numéricos.' });
                  return;
                }
                const success = addCustomAuthorizedCode(clean);
                if (success) {
                  setCodeAuthorizeMsg({ type: 'success', text: `¡Código #${clean} autorizado exitosamente!` });
                  setNewManualCodeToAuthorize('');
                  reloadUsers(false);
                } else {
                  setCodeAuthorizeMsg({ type: 'error', text: 'No se pudo autorizar el código.' });
                }
              }}
              className="bg-slate-950/80 p-3 rounded-2xl border border-slate-800 space-y-2"
            >
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-slate-300 flex items-center gap-1.5">
                  <Plus className="w-3.5 h-3.5 text-cyan-400" />
                  <span>Autorizar Nuevo Código (6 Dígitos)</span>
                </label>
                <span className="text-[10px] text-slate-400">Entrega inmediata a compradora</span>
              </div>
              <div className="flex gap-2">
                <input
                  type="text"
                  maxLength={6}
                  value={newManualCodeToAuthorize}
                  onChange={(e) => setNewManualCodeToAuthorize(e.target.value.replace(/\D/g, ''))}
                  placeholder="Ej: 125294 o 138371"
                  className="flex-1 bg-slate-900 border border-slate-700 focus:border-cyan-400 rounded-xl px-3 py-2 text-xs text-cyan-300 font-mono font-bold tracking-widest"
                />
                <button
                  type="submit"
                  className="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-bold rounded-xl transition-colors cursor-pointer flex items-center gap-1.5 shadow-sm"
                >
              ...
              {codeAuthorizeMsg && (
                <div className={`p-2 rounded-xl text-xs flex items-center gap-2 ${
                  ...
                  <span>{codeAuthorizeMsg.text}</span>
                </div>
              )}
            </form>
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-1 min-h-[220px]">
              {codesSummary.masterList
                .filter(code => {
                  ...
                  return code.includes(query) || (info && (info.userName?.toLowerCase().includes(query) || info.userEmail?.toLowerCase().includes(query)));
                })
                .map((code) => {
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {isRedeemed ? (
                              <span className="px-2 py-0.5 bg-slate-800 border border-slate-700 text-slate-300 text-[10px] font-bold rounded-md">
                                Canjeado
                              </span>
                            ) : (
                              <span className="px-2 py-0.5 bg-emerald-950 border border-emerald-600/50 text-emerald-300 text-[10px] font-bold rounded-md flex items-center gap-1">
                          ...
                          {isRedeemed ? (
                            <p className="text-[11px] text-slate-400 mt-0.5">
                              Usuaria: <strong className="text-white">{redemptionInfo.userName}</strong>
                              {redemptionInfo.userPhone && ` • WA: ${redemptionInfo.userPhone}`}
                            </p>
                          ) : (
                            <p className="text-[11px] text-slate-400 mt-0.5">

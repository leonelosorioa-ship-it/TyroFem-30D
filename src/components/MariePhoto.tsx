  ...
  return (
    <div className={`relative inline-block select-none ${className}`}>
      ...
      {(size === 'lg' || size === 'xl' || size === 'hero') && (
        <div className="absolute -inset-2 bg-gradient-to-r from-cyan-500/30 via-teal-500/20 to-emerald-500/30 rounded-full blur-xl pointer-events-none animate-pulse" />
      )}

      {/* Main Container Frame with circular dark glass frame */}
      <div 
        className={`relative ${dimensions} rounded-full overflow-hidden border-2 border-cyan-400/90 shadow-[0_4px_25px_rgba(0,229,255,0.4)] bg-[#070e14] flex items-center justify-center`}
      >
        {/* Exact Circulo Marie Photo */}
        <img
        ...
        {showBadge && (
          <div className="absolute bottom-1 z-20">
            <span className={`${badgeTextSize} font-bold rounded-full bg-slate-950/95 text-cyan-300 border border-cyan-400/70 shadow-lg backdrop-blur-xs flex items-center gap-1`}>
      ...
      <div className="absolute top-0 right-0 z-30 flex items-center justify-center">
        <span className="w-3.5 h-3.5 rounded-full bg-emerald-400 ring-2 ring-[#0a1520] shadow-[0_0_10px_#34d399]" />
      </div>
    </div>
  );
};
